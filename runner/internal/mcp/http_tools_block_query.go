...
func (s *HTTPServer) createMemoryRetrieveTool() *MCPTool {
	return &MCPTool{
		...
		Description: "Semantic search over the workspace: returns blocks (notes/tasks/comments) ranked by relevance to the query. Use for long-term memory lookup before drafting a response.",
...
func (s *HTTPServer) createBlockListTypesTool() *MCPTool {
	return &MCPTool{
		...
		Description: "List every block type available in the workspace (bootstrap + indicator.define registrations). Returns the hydrated type spec for each (columns, required keys, default view, etc.). Call this first when the target type might have been defined dynamically by an agent.",
		InputSchema: map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
				"workspace_id": blockstoreStringProp("Workspace UUID."),
