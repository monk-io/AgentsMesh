package claude

import (
	"bufio"
	"encoding/json"
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

// TestTransport_SendControlRequest_Interrupt tests the full round-trip:
// Transport sends interrupt control_request → stdin → mock CLI responds → Transport returns result.
func TestTransport_SendControlRequest_Interrupt(t *testing.T) {
	f := newFixtureWithStdin()
	defer f.Close()

	// Simulate handshake completion so transport is ready.
	writeLine(f.PW, map[string]any{
		"type": "control_response",
		"response": map[string]any{"subtype": "success", "request_id": "init_1"},
	})
	f.Drain()

	// Launch goroutine to read the outgoing control_request from stdin and respond.
	go func() {
		scanner := bufio.NewScanner(f.StdinPR)
		for scanner.Scan() {
			var msg map[string]any
			if err := json.Unmarshal(scanner.Bytes(), &msg); err != nil {
				continue
			}
			if msg["type"] == "control_request" {
				req := msg["request"].(map[string]any)
				if req["subtype"] == "interrupt" {
					// Send matching control_response via stdout pipe.
					writeLine(f.PW, map[string]any{
						"type": "control_response",
						"response": map[string]any{
							"subtype":    "success",
							"request_id": msg["request_id"],
							"response":   map[string]any{"interrupted": true},
						},
					})
					return
				}
			}
		}
	}()

	resp, err := f.Transport.SendControlRequest("", "interrupt", nil)
	require.NoError(t, err)
	assert.Equal(t, true, resp["interrupted"])
}

// TestTransport_SendControlRequest_SetPermissionMode tests set_permission_mode with payload.
func TestTransport_SendControlRequest_SetPermissionMode(t *testing.T) {
	f := newFixtureWithStdin()
	defer f.Close()

	// Handshake.
	writeLine(f.PW, map[string]any{
		"type": "control_response",
		"response": map[string]any{"subtype": "success", "request_id": "init_1"},
	})
	f.Drain()

	// Mock CLI: read request, verify payload, respond.
	go func() {
		scanner := bufio.NewScanner(f.StdinPR)
		for scanner.Scan() {
			var msg map[string]any
			if err := json.Unmarshal(scanner.Bytes(), &msg); err != nil {
				continue
			}
			if msg["type"] == "control_request" {
				req := msg["request"].(map[string]any)
				// Verify payload fields are present.
				if req["subtype"] == "set_permission_mode" && req["mode"] == "acceptEdits" {
					writeLine(f.PW, map[string]any{
						"type": "control_response",
						"response": map[string]any{
							"subtype":    "success",
							"request_id": msg["request_id"],
							"response":   map[string]any{"mode": "acceptEdits"},
						},
					})
					return
				}
			}
		}
	}()

	resp, err := f.Transport.SendControlRequest("", "set_permission_mode", map[string]any{"mode": "acceptEdits"})
	require.NoError(t, err)
	assert.Equal(t, "acceptEdits", resp["mode"])
}

// TestTransport_SendControlRequest_ErrorResponse tests handling of error control_response.
func TestTransport_SendControlRequest_ErrorResponse(t *testing.T) {
	f := newFixtureWithStdin()
	defer f.Close()

	writeLine(f.PW, map[string]any{
		"type": "control_response",
		"response": map[string]any{"subtype": "success", "request_id": "init_1"},
	})
	f.Drain()

	go func() {
		scanner := bufio.NewScanner(f.StdinPR)
		for scanner.Scan() {
			var msg map[string]any
			if err := json.Unmarshal(scanner.Bytes(), &msg); err != nil {
				continue
			}
			if msg["type"] == "control_request" {
				writeLine(f.PW, map[string]any{
					"type": "control_response",
					"response": map[string]any{
						"subtype":    "error",
						"request_id": msg["request_id"],
						"error":      "model not available",
					},
				})
				return
			}
		}
	}()

	_, err := f.Transport.SendControlRequest("", "set_model", map[string]any{"model": "invalid"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "model not available")
}

// TestTransport_SendControlRequest_ConcurrentWithPermission tests that outgoing control_requests
// don't interfere with incoming permission control_requests flowing in the opposite direction.
func TestTransport_SendControlRequest_ConcurrentWithPermission(t *testing.T) {
	f := newFixtureWithStdin()
	defer f.Close()

	writeLine(f.PW, map[string]any{
		"type": "control_response",
		"response": map[string]any{"subtype": "success", "request_id": "init_1"},
	})
	f.Drain()

	// Respond to outgoing interrupt, then send an incoming permission request.
	go func() {
		scanner := bufio.NewScanner(f.StdinPR)
		for scanner.Scan() {
			var msg map[string]any
			if err := json.Unmarshal(scanner.Bytes(), &msg); err != nil {
				continue
			}
			if msg["type"] == "control_request" {
				// 1. Respond to interrupt
				writeLine(f.PW, map[string]any{
					"type": "control_response",
					"response": map[string]any{
						"subtype":    "success",
						"request_id": msg["request_id"],
						"response":   map[string]any{},
					},
				})
				// 2. Send incoming permission request (different direction)
				time.Sleep(10 * time.Millisecond)
				writeLine(f.PW, map[string]any{
					"type":       "control_request",
					"request_id": "perm-incoming-1",
					"request": map[string]any{
						"subtype":   "can_use_tool",
						"tool_name": "Edit",
						"input":     map[string]any{},
					},
				})
				return
			}
		}
	}()

	// Outgoing interrupt should succeed.
	_, err := f.Transport.SendControlRequest("", "interrupt", nil)
	require.NoError(t, err)

	// Incoming permission request should arrive independently.
	f.Drain()
	f.mu.Lock()
	defer f.mu.Unlock()
	require.Len(t, f.PermissionRequests, 1)
	assert.Equal(t, "Edit", f.PermissionRequests[0].ToolName)
}
