...
func TestTransport_Handshake(t *testing.T) {
	...
	// Mock server: read the initialize request, respond, then drain remaining writes
	go func() {
		...
		// Drain the "initialized" notification that Handshake writes after getting the response
...
func TestTransport_SendPrompt(t *testing.T) {
	...
	// Read the request on the stdin side
	...
	go func() {
		...
		json.Unmarshal(scanner.Bytes(), &req)
		received <- req
		// Respond so the background goroutine cleans up
