...
impl WasmAutopilotState {
    ...
    pub fn replace_cached_controllers(&self, req_bytes: &[u8]) -> Result<(), JsValue> {
        let req = ReplaceCachedControllersRequest::decode(req_bytes).map_err(decode_err)?;
        self.state.write().autopilot.set_controllers(req.controllers.into_iter().map(from_snapshot).collect());
        Ok(())
    }
    ...
    pub fn replace_cached_iterations(&self, req_bytes: &[u8]) -> Result<(), JsValue> {
        ...
        self.state.write().autopilot.set_iterations(req.autopilot_controller_key, iters);
        Ok(())
    }
