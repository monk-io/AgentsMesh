...
export function useRunnerDetail(t: (key: string) => string) {
  ...
  const loadRunner = useCallback(async () => {
    try {
      ...
      const runner = res.runner ?? res;
      setRunner(runner);
      setRelayConnections(res.relay_connections || runner.relay_connections || []);
      setLatestRunnerVersion(res.latest_runner_version || runner.latest_runner_version);
    } catch (error) {
